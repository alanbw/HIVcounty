...
county.tbl1 = county.df %>% 
  ...
  tbl_summary(     
    ...
    label  = list(                                              # display labels for column names
      ...
      percent_clustered_2022_cur ~ "Percentage in cluster",
        zc_income_cur ~ "Median income",
       predictor_zc_per_poverty_cur ~ "Percentage below poverty level",
      predictor_zc_per_hispanic_cur ~ "Percentage Hispanic",
      predictor_zc_per_education_cur ~ 'Percentage high school education',
      predictor_zc_per_race_cur ~ 'Percentage White',
      predictor_zc_per_employment_cur ~ 'Percentage employed'),
...
county.rsd_zip.tbl1 = county.df %>% 
  ...
  tbl_summary(     
    ...
    type   = all_categorical() ~ "categorical",                 # force all categorical levels to display
    label  = list(                                              # display labels for column names
      percent_clustered_2022_rsd   ~ "Percent clustered (2022)",                           
      zc_income_rsd ~ "Median income",
      predictor_zc_per_poverty_rsd ~ "Percentage below poverty level",
      predictor_zc_per_hispanic_rsd ~ "Percentage Hispanic",
      predictor_zc_per_education_rsd ~ 'Percentage high school education or higher',
      predictor_zc_per_race_rsd ~ 'Percentage White',
      predictor_zc_per_employment_rsd ~ 'Percentage employed'),
    missing_text = "Missing"                                    # how missing values should display
  )
...
county.cur_zip.tbl1 = county.df %>% 
  ...
  tbl_summary(     
    ...
    type   = all_categorical() ~ "categorical",                 # force all categorical levels to display
    label  = list(                                              # display labels for column names
      percent_clustered_2022_cur   ~ "Percent clustered (2022)",                           
      zc_income_cur ~ "Median income",
      predictor_zc_per_poverty_cur ~ "Percentage below poverty level",
      predictor_zc_per_hispanic_cur ~ "Percentage Hispanic",
      predictor_zc_per_education_cur ~ 'Percentage high school education or higher',
      predictor_zc_per_race_cur ~ 'Percentage White',
      predictor_zc_per_employment_cur ~ 'Percentage employed'),
    missing_text = "Missing"                                    # how missing values should display
  )
...
reg_variable.vec_rsd = reg_variable.vec_rsd[-which(reg_variable.vec_rsd =="zc_income_rsd")]
reg_variable.vec_cur = reg_variable.vec_cur[-which(reg_variable.vec_cur =="zc_income_cur")]
...
#   breaks_qt <- classIntervals(zcta_level.df$per_outcome, n = 2, style = "quantile")
#   colorPal <- colorQuantile("YlOrRd", zcta_level.df$per_outcome,n = 2)
#   pal_fun(zcta_level.df$per_outcome) %>% unique()
#   quantile.bins <- quantile(zcta_level.df$per_outcome, 
#                                    probs = seq(0, 1,length.out = break_num + 1),
#                                    na.rm = TRUE)
#   cut(zcta_level.df$per_outcome, quantile.bins, labels = NULL, include.lowest = TRUE, 
#         right = FALSE)
#   zcta_level.df = zcta_level.df %>%
#     mutate(color = pal_fun(per_outcome),
#            quantile.bin = cut(per_outcome, quantile.bins, labels = FALSE, include.lowest = TRUE, 
#         right = FALSE))
#   
#   colors.legend <- zcta_level.df %>%
#     arrange(per_outcome) %>%
#     distinct(quantile.bin,color)
#   
...
#       stroke = F, 
#       fillColor = ~color,
#        # fillColor = ~pal_fun(n_outcome),
#       # fillColor = ~colorPal(n_outcome),
#       fillOpacity = 0.8, smoothFactor = 0.5,
#       popup = p_popup) %>%
#     addTiles()  %>%
#      # addLegend(
#      #      pal = colorPal,
#      #      values = zcta_level.df$per_outcome,
#      #      opacity = 0.3,
#      #       labels = paste0("up to ", format(breaks_qt$brks[-1], digits = 2)),
#      #      # title = input$selectzipcolor,
#      #      position = "bottomleft"
#      #    ) %>%
#     addLegend("bottomright",
#               # colors = brewer.pal(break_num, "YlOrRd"),
#               colors = colors.legend %>% filter(!is.na(quantile.bin)) %>% pull(color),
#               labels = paste0("up to ", format(breaks_qt$brks[-1], digits = 2)),
#               title =  title_a) %>%
